...
ADV_VAL=R/'advanced_selected_validation_regression_oof.csv'
ADV_23=R/'advanced_selected_2023_regression.csv'
...
def main():
    if not ADV_VAL.exists() or not ADV_23.exists():raise FileNotFoundError('advanced bridges missing')
    a=prep(ADV_VAL,'adv_pred');a23=prep(ADV_23,'adv_pred')
    searches=[]
    for bname,bdir in BRANCHES.items():
        vp=bdir/'selected_validation_oof.csv'
        if not vp.exists():continue
        o=prep(vp,'orig_pred');m=join(a,o,'orig_pred')
    ...
    for _,r in sel.iterrows():
        ...
        o23=prep(p23,'orig_pred');m23=join(a23,o23,'orig_pred');q=m23[m23.position.eq(r.position)].copy();w=float(r.original_weight);q['pred']=(1-w)*q.adv_pred+w*q.orig_pred
