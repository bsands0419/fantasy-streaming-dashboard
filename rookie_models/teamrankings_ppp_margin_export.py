...
def parse(season,date):
 ...
 for table in soup.find_all('table'):
  ...
  headers_seen=headers;team_i=hlow.index('team');rank_i=hlow.index('rank')
  # TeamRankings season pages sometimes label the full-season column as the prior calendar year,
  # and sometimes omit the year label. We never blindly trust column 3 without validation.
  value_i=headers.index(str(season)) if str(season) in headers else None
  for tr in table.find_all('tr'):
   cells=[re.sub(r'\s+',' ',td.get_text(' ',strip=True)) for td in tr.find_all('td')]
   if cells and len(debug)<8: debug.append(cells)
  if season in {2006,2007,2008,2009,2010}:
   print('DEBUG',season,'HEADERS',headers_seen,'ROWS',debug[:4])
  if value_i is None:
   raise RuntimeError(f'PPP margin {season}: season column not explicitly present; headers={headers_seen}; sample={debug[:3]}')
  for tr in table.find_all('tr'):
   cells=[re.sub(r'\s+',' ',td.get_text(' ',strip=True)) for td in tr.find_all('td')]
 ...
 if not d['points_per_play_margin'].between(-2,2).all(): raise RuntimeError(f'PPP margin {season} out of expected range; headers={headers_seen}; min={d.points_per_play_margin.min()} max={d.points_per_play_margin.max()}')
 return d
def main():
 frames=[];audit=[]
 for season,date in FINAL_SNAPSHOT.items():
  d=parse(season,date);frames.append(d);audit.append({'college_season':season,'source_date':date,'teams':len(d),'min':d.points_per_play_margin.min(),'max':d.points_per_play_margin.max(),'status':'ok'});print(season,len(d),d.points_per_play_margin.min(),d.points_per_play_margin.max())
