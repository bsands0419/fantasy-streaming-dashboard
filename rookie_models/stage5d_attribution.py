...
def pretty_feature(feature: str) -> str:
    ...
    for prefix in ["pass_", "rush_", "rec_", "rb_", "qb_", "landing_"]:
        if s.startswith(prefix):
            ...
            break
    labels = {
        "draft_round": "draft round", "draft_pick": "draft pick", "log_pick": "log draft pick",
        "pick_inv_sqrt": "inverse-root draft pick", "day1": "first-round indicator", "day2": "day-two indicator",
        "draft_age": "draft age", "young_for_position": "youth for position", "yardsdropback": "yards per dropback",
        "epaplay": "EPA per play", "epagame": "EPA per game", "yardsgame": "yards per game",
        "yardsplay": "yards per play", "comppct": "completion percentage", "sack_rate": "sack rate",
        "int_rate": "interception rate", "pass_td_rate": "passing TD rate",
        "qb_rush_fantasy_pg": "QB rushing fantasy points per game", "rush_share": "rushing share",
        "rush_yard_share": "rushing-yard share", "target_share": "target share",
        "rec_yard_share": "receiving-yard share", "rec_td_share": "receiving-TD share",
        "reception_share": "reception share", "yards_per_target": "yards per target",
        "targets_per_game": "targets per game", "rec_per_game": "receptions per game",
        ...
        "scrim_yards_per_team_play": "scrimmage yards per team play", "scrim_ypg": "scrimmage yards per game",
        "rec_yards_per_team_pass": "receiving yards per team pass attempt", "speed_score_v2": "speed score",
        "burst_proxy": "burst proxy",
    }
    base = labels.get(s, s.replace("_", " "))
...
def score_batch(x: pd.DataFrame, job) -> np.ndarray:
    a, bb = job["fitted"]["primary_ppg"]
    pa = np.asarray(a.predict(x[job["features"]]), float)
    if bb is None:
        return pa
    pb = np.asarray(bb.predict(x[job["features_b"]]), float)
    w = float(job["blend"]["weight"])
    return w * pa + (1.0 - w) * pb


def hit_batch(x: pd.DataFrame, job) -> np.ndarray:
    return np.asarray(job["classifier"].predict_proba(x[job["features"]])[:, 1], float)
...
def make_family_batch(one: pd.DataFrame, fams: list[str], all_feats: list[str], medians: dict[str, float]) -> tuple[pd.DataFrame, list[list[str]]]:
    ...
    for i, fam in enumerate(fams):
        ...
        fam_features.append(fs)
        for f in fs:
            batch.at[i, f] = medians[f]
    return batch, fam_features
...
def main():
    ...
    feat_rows, family_rows, audits, explanation_rows = [], [], [], []
    ...
    for pos in POSITIONS:
        ...
        h = add_missing_columns(hist[hist.position.eq(pos)].copy().reset_index(drop=True), all_feats)
        c = add_missing_columns(cur[cur.position.eq(pos)].copy().reset_index(drop=True), all_feats)
        medians = {f: num(pd.to_numeric(h[f], errors="coerce").median()) for f in all_feats}
        ...
        for _, player in c.iterrows():
            ...
            listed_score = num(rr.iloc[0].pred_best2of3_ppg) if len(rr) else np.nan
            listed_hit = num(rr.iloc[0].hit_probability) if len(rr) else np.nan
            ...
            valid_features = [f for f in all_feats if np.isfinite(medians[f])]
            fb = make_feature_batch(one, valid_features, medians)
            f_scores = score_batch(fb, job)
            f_hits = hit_batch(fb, job)
            player_feature_rows = []
            for i, f in enumerate(valid_features):
                raw = num(one.iloc[0][f])
                rec = {
                    "position": pos, "pfr_name": name, "feature": f, "pretty_feature": pretty_feature(f),
                    ...
                    "raw_value": raw, "historical_median": medians[f], "historical_nonmissing_n": counts[f],
                    "raw_value_percentile": percentile(h[f], raw), "raw_value_missing": int(not np.isfinite(raw)),
                    "base_pred_best2of3_ppg": base_score, "neutralized_pred_best2of3_ppg": float(f_scores[i]),
                    "score_delta_ppg": base_score - float(f_scores[i]), "base_hit_probability": base_hit,
                    "neutralized_hit_probability": float(f_hits[i]), "hit_delta_pp": 100.0 * (base_hit - float(f_hits[i])),
                }
                rec["direction_conflict"] = int(np.sign(rec["score_delta_ppg"]) != 0 and np.sign(rec["hit_delta_pp"]) != 0 and np.sign(rec["score_delta_ppg"]) != np.sign(rec["hit_delta_pp"]))
                feat_rows.append(rec)
                player_feature_rows.append(rec)
            pfd = pd.DataFrame(player_feature_rows)

            fams = sorted({family(f) for f in valid_features})
            famb, fam_features = make_family_batch(one, fams, all_feats, medians)
            fam_scores = score_batch(famb, job)
            fam_hits = hit_batch(famb, job)
            player_family_rows = []
            for i, fam in enumerate(fams):
                fs = fam_features[i]
                hit_fs = [f for f in fs if f in hp_feats]
                frec = {
                    "position": pos, "pfr_name": name, "feature_family": fam,
                    "family_feature_count": len(fs), "family_hit_feature_count": len(hit_fs),
                    "base_pred_best2of3_ppg": base_score, "neutralized_pred_best2of3_ppg": float(fam_scores[i]),
                    "score_delta_ppg": base_score - float(fam_scores[i]), "base_hit_probability": base_hit,
                    "neutralized_hit_probability": float(fam_hits[i]), "hit_delta_pp": 100.0 * (base_hit - float(fam_hits[i])),
                }
                frec["direction_conflict"] = int(np.sign(frec["score_delta_ppg"]) != 0 and np.sign(frec["hit_delta_pp"]) != 0 and np.sign(frec["score_delta_ppg"]) != np.sign(frec["hit_delta_pp"]))
            ...
            explanation_rows.append({
                "position": pos, "rank": meta.get("rank"), "pfr_name": name,
                "prospect_model_percentile": meta.get("prospect_model_percentile"),
                "pred_best2of3_ppg": base_score, "hit_probability": base_hit,
                ...
                "diagnostic_priority": meta.get("diagnostic_priority"), "diagnostic_notes": meta.get("diagnostic_notes"),
        ...
        pred_arr = pd.to_numeric(pd.Series(pred_errs), errors="coerce")
        prob_arr = pd.to_numeric(pd.Series(prob_errs), errors="coerce")
        max_pred_err = float(pred_arr.max()) if pred_arr.notna().any() else np.nan
        max_prob_err = float(prob_arr.max()) if prob_arr.notna().any() else np.nan
        audits.append({
            "position": pos, "current_rows": len(c), "historical_rows": len(h),
            "regression_primary_features": len(job.get("features", [])),
            "regression_secondary_features": len(job.get("features_b") or []), "union_features": len(all_feats),
            "hit_classifier_features": len(hp_feats), "max_abs_regression_reproduction_error": max_pred_err,
    ...
    for _, r in explanations[explanations.diagnostic_priority.eq("High")].iterrows():
        report.append(f"### {r.pfr_name} ({r.position})\n")
        report.append(f"- Grade: {num(r.prospect_model_percentile):.1f}th percentile; projected best-2-of-3 PPG: {num(r.pred_best2of3_ppg):.2f}; classifier hit: {100*num(r.hit_probability):.1f}%.\n")
    ...
    manifest = {
        "generated_utc": datetime.now(timezone.utc).isoformat(), "stage": "5D",
        "model_changed": False, "model_selection_changed": False,
        "method": "Batched local historical-median neutralization sensitivity on exact frozen Stage 3 regression and hit classifier, per feature and football feature family.",
        "interpretation": "Positive delta means the observed input raises the frozen output versus replacing that input with the historical same-position median. Effects are local, non-causal, and non-additive.",
        "outputs": ["feature_attribution_2026.csv", "family_attribution_2026.csv", "prospect_explanations_2026.csv", "model_reproduction_audit.csv", "REPORT.md"],
        "inputs": ["results_v3/prospect_pool_v3.csv", "results_2026/prospect_pool_2026.csv", "results_stage5c/rookie_rankings_2026_stage5c.csv", "trained_v3/{position}.joblib"],
    }
